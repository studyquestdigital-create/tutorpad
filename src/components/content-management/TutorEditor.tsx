...
import { User, Plus, Trash2, Loader } from 'lucide-react';
import toast from 'react-hot-toast';
import { Tutor, Subject } from '../../types';
import * as api from '../../lib/api';

const TutorEditor: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const [tutorsData, subjectsData] = await Promise.all([api.getTutors(), api.getSubjects()]);
      ...
      <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4 mb-6 border border-gray-200 dark:border-gray-600">
        <form className="space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">This is a simplified view. Full assignment functionality coming soon.</p>
            <select multiple className="w-full p-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 h-32" disabled>
              {subjects.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
            </select>
          </div>
          <motion.button type="submit" whileTap={{ scale: 0.98 }} className="w-full flex items-center justify-center gap-2 bg-primary-600 text-white px-4 py-2 rounded-lg hover:bg-primary-700" disabled>
